...
function NavLink({
  href,
  icon,
  activeClassName = '[&>svg]:fill-white [&>svg>path]:stroke-white',
  isActive,
}: Readonly<{
  href: string;
  ...
  return (
    <Link
      href={href}
      className={cn('transition-all', isActive ? activeClassName : '')}
    >
      {icon}
    </Link>
  );
...
export default function MobileBottomNav() {
  ...
  return (
    <nav className="fixed bottom-6 left-6 right-6 rounded-2xl z-50 bg-primary h-14 flex justify-around items-center shadow-md">
      ...
      <NavLink
        href="/word-lists"
        icon={<BookAIcon className="size-8 text-white stroke-1" />}
        activeClassName=""
        isActive={pathname === '/word-lists'}
      />
      <NavLink
        href="/profile"
        icon={
